        ...
        <div class="card-body">
          ...
          <div class="card-footer">
            <div>
              ...
              ${isRemate ? `<div style="font-size:0.7rem;font-weight:700;color:var(--accent2);letter-spacing:1px;margin-bottom:2px;">🔨 💰 REMATE</div><div class="card-precio">Precio base: ${precio}</div>` : `<div class="card-precio">${precio}</div>`}
            </div>
    ...
    return `
      <div class="admin-item">
        ...
        <div class="admin-item-info">
          ...
          <div class="admin-item-meta">${antStr}${precioStr} ${descBadge} · ${p.categoria||'Sin categoría'}${p.condicion ? ' · ' + (p.condicion === 'Nuevo' ? '✨ Nuevo' : '♻️ Usado') : ''}</div>
          ${ofertaHtml}
          <span class="pill ${pillClass}" style="margin-top:6px">${pillLabel}</span>
          <div class="admin-orden-wrap" style="margin-top:8px;display:flex;align-items:center;gap:6px;">
            <label style="font-size:0.75rem;color:var(--text2);" for="orden-${p.id}">Orden:</label>
            <input type="number" id="orden-${p.id}" value="${p.orden ?? ''}" placeholder="—" style="width:64px;padding:4px 8px;font-size:0.85rem;" onchange="guardarOrden('${p.id}', this.value)">
          </div>
        </div>
...
// Guarda el orden manual del producto. Vacío = sin orden (va al final).
async function guardarOrden(id, valor) {
  const orden = valor === '' ? null : Number(valor);
  await db.collection('productos').doc(id).update({ orden });
