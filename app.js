...
auth.setPersistence(firebase.auth.Auth.Persistence.LOCAL).catch(e => console.log('Persistencia auth:', e));
...
function cargarProductosTienda() {
  ...
  // Devolvemos una promesa que resuelve en la PRIMERA carga, para poder
  // esperarla antes de restaurar la última pantalla vista.
...
async function cargarProductosAdmin() {
  ...
  lista.innerHTML = productosCache.map(p => {
    ...
    // Price display — en remate mostramos el precio BASE, no el precio normal
    const precioMostrarAdmin = (p.esRemate && p.precioBase) ? p.precioBase : p.precio;
    const precioStr  = (p.esRemate ? 'Base: ' : '') + formatPrecio(precioMostrarAdmin, p.moneda);
    const antStr     = p.precioAnterior ? `<span class="admin-precio-tachado">${formatPrecio(p.precioAnterior, p.moneda)}</span> ` : '';
    const descPct    = p.precioAnterior ? Math.round((1 - precioMostrarAdmin/p.precioAnterior)*100) : 0;
