    ...
    return `
      <div class="admin-item" draggable="true" data-id="${p.id}" ondragstart="dragStartProducto(event)" ondragover="dragOverProducto(event)" ondrop="dragDropProducto(event)" ondragend="dragEndProducto(event)">
        ...
        <div class="admin-item-info">
          ...
          <div class="admin-item-meta">${antStr}${precioStr} ${descBadge} · ${p.categoria||'Sin categoría'}${p.condicion ? ' · ' + (p.condicion === 'Nuevo' ? '✨ Nuevo' : '♻️ Usado') : ''}${p.esRemate && p.precioBase ? ' · <span style="color:#a78bfa;font-weight:600;">🔨 Base: $' + Number(p.precioBase).toLocaleString('es-AR') + '</span>' : ''}</div>
          ${ofertaHtml}
